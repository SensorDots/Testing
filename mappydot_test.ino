/**
   Basic MappyDot Test Application
...
void measurement_mode_default()
{
  ...
  Wire.write(VL53L0X_DEFAULT);
  Wire.endTransmission();
}

void measurement_mode_high_speed()
{
  Wire.beginTransmission(address);
  Wire.write(RANGING_MEASUREMENT_MODE);
  Wire.write(HIGH_SPEED);
  Wire.endTransmission();
}


void measurement_mode_long_range()
{
  Wire.beginTransmission(address);
  Wire.write(RANGING_MEASUREMENT_MODE);
...
{
  Wire.beginTransmission(address);
  Wire.write(RESET_VL53L0X_RANGING);
  Wire.endTransmission();
}

void shutdown_vl53l0x()
{
  Wire.beginTransmission(address);
  Wire.write(VL53L0X_SHUTDOWN);
  Wire.endTransmission();
}

void startup_vl53l0x()
{
  Wire.beginTransmission(address);
  Wire.write(VL53L0X_NOT_SHUTDOWN);
  Wire.endTransmission();
}
...
void print_menu()
{
  ...
  Serial.println("D - measurement_mode_default");
  ...
  Serial.println("g - shutdown_vl53l0x");
  Serial.println("G - startup_vl53l0x");
...
void loop() {
  ...
  switch (Serial.read())
  {
    ...
    case 'D': measurement_mode_default(); break;
    case 'H': measurement_mode_high_speed(); break;
    case 'L': measurement_mode_long_range(); break;
    case 'A': measurement_mode_high_accurate(); break;
    case 'I': intersensor_crosstalk_enable(); break;
    case 'i': intersensor_crosstalk_disable(); break;
    case 'V': reset_vl53l0x(); break;
    case 'g': shutdown_vl53l0x(); break;
    case 'G': startup_vl53l0x(); break;
